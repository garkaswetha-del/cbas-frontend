...
function computeTemplateDate(mmdd: string, academicYear: string): string {
...
interface CalendarEvent {
  id: string; academic_year: string; title: string; event_type: string;
  start_date: string; end_date: string; applies_to: string; grade: number | null;
}

interface Template {
  id: string; title: string; event_type: string; category: string;
  default_mmdd: string | null; multi_day: boolean; display_order: number;
}

const BLANK_FORM = {
  title: '', event_type: 'holiday', start_date: '', end_date: '',
  applies_to: 'all', grade: '' as string | number,
};

const BLANK_TMPL_FORM = {
  title: '', event_type: 'holiday', category: '', default_mmdd: '', multi_day: false,
};
...
export default function AcademicCalendarPage({ readOnly = false }: { readOnly?: boolean }) {
  ...
  const [templateAdd,   setTemplateAdd]   = useState<Template | null>(null);
  ...
  const [manageMode,    setManageMode]   = useState<'list' | 'form'>('list');
  ...
  const loadTemplates = useCallback(async () => {
    try {
      const res = await axios.get(`${API}/calendar/templates`);
  ...
  useEffect(() => {
    ...
    function onDown(e: MouseEvent) {
      if (quickPopoverRef.current    && !quickPopoverRef.current.contains(e.target as Node))    setQuickAdd(null);
      if (templatePopoverRef.current && !templatePopoverRef.current.contains(e.target as Node)) setTemplateAdd(null);
    }
  ...
  function openQuickAdd(date: string, e: React.MouseEvent) {
    ...
    setQuickTitle(''); setQuickType('holiday'); setQuickEndDate(date);
    setQuickAdd({ date, x: Math.min(rect.left, window.innerWidth - 300), y: Math.min(rect.bottom + 4, window.innerHeight - 220) });
  }
  ...
  async function doQuickSave() {
    ...
    try {
      await axios.post(`${API}/calendar`, {
        academic_year: academicYear, title: quickTitle.trim(), event_type: quickType,
        start_date: quickAdd.date, end_date: quickEndDate || quickAdd.date,
        applies_to: 'all', grade: null,
      });
      setQuickAdd(null); await load();
    } finally { setQuickSaving(false); }
  }

  function openMoreOptions() {
    if (!quickAdd) return;
    setForm({ title: quickTitle, event_type: quickType, start_date: quickAdd.date,
              end_date: quickEndDate || quickAdd.date, applies_to: 'all', grade: '' });
    setModal({ mode: 'add' }); setQuickAdd(null);
  }

  // ── template chip handlers ──

  function openTemplatePick(t: Template) {
    const def = t.default_mmdd ? computeTemplateDate(t.default_mmdd, academicYear) : '';
    setTmplStart(def); setTmplEnd(def); setTemplateAdd(t);
  }

  async function doTmplSave() {
    if (!templateAdd || !tmplStart || tmplSaving) return;
    setTmplSaving(true);
    try {
      await axios.post(`${API}/calendar`, {
        academic_year: academicYear, title: templateAdd.title, event_type: templateAdd.event_type,
        start_date: tmplStart, end_date: tmplEnd || tmplStart, applies_to: 'all', grade: null,
      });
      setTemplateAdd(null); await load();
    } finally { setTmplSaving(false); }
  }

  // ── full event modal ──

  function openAdd() { setForm({ ...BLANK_FORM }); setModal({ mode: 'add' }); }

  function openEdit(ev: CalendarEvent) {
    setForm({ title: ev.title, event_type: ev.event_type,
              start_date: ev.start_date.slice(0, 10), end_date: ev.end_date.slice(0, 10),
              applies_to: ev.applies_to, grade: ev.grade ?? '' });
  ...
  async function modalSave() {
    ...
    setSaving(true);
    try {
      const payload = { ...form, academic_year: academicYear, grade: form.grade !== '' ? Number(form.grade) : null };
      if (modal?.mode === 'edit' && modal.event) await axios.patch(`${API}/calendar/${modal.event.id}`, payload);
      else                                        await axios.post(`${API}/calendar`, payload);
  ...
  async function remove(id: string) {
    ...
    await axios.delete(`${API}/calendar/${id}`); await load();
  }

  // ── manage templates ──

  function openManage() { setManageMode('list'); setEditingTmpl(null); setManageOpen(true); }

  function openTmplForm(t?: Template) {
    setEditingTmpl(t ?? null);
  ...
  async function deleteTmpl(id: string) {
    if (!window.confirm('Remove this template chip?')) return;
    await axios.delete(`${API}/calendar/templates/${id}`); await loadTemplates();
  }

  // Group templates by category
  const tmplCategories = Array.from(new Set(templates.map(t => t.category)));
  const existingCategories = tmplCategories;

  // ── calendar grid ──

  const currentMonth  = months[currentMonthIdx];
  const cy = currentMonth.getFullYear(), cm = currentMonth.getMonth();
    ...
    <div className="p-4 max-w-6xl mx-auto">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {!readOnly && <button onClick={openAdd} className="bg-indigo-600 hover:bg-indigo-700 text-white text-sm px-4 py-1.5 rounded font-medium">+ Add Event</button>}
        </div>
      </div>
      ...
      {!readOnly && (
        <div className="mb-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs font-semibold text-gray-500">Quick add common events:</p>
            <button onClick={openManage} className="text-xs text-indigo-600 hover:text-indigo-800 font-medium flex items-center gap-1">
              ⚙ Manage
            </button>
          </div>
          {templates.length === 0 ? (
            <p className="text-xs text-gray-400">No templates yet. Click Manage to add some.</p>
          ) : (
            <div className="flex flex-wrap gap-1.5">
              {templates.map(t => {
                const td = TYPE_MAP[t.event_type];
                return (
                  <button key={t.id} onClick={() => openTemplatePick(t)}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 bg-gray-50">
            <button onClick={() => setCurrentMonthIdx(i => Math.max(0, i - 1))} disabled={currentMonthIdx === 0}
              className="p-1.5 rounded hover:bg-gray-200 disabled:opacity-30 text-gray-600 text-lg leading-none">‹</button>
          ...
          <div className="flex overflow-x-auto border-b border-gray-200 bg-gray-50">
            {months.map((m, idx) => (
              <button key={idx} onClick={() => setCurrentMonthIdx(idx)}
                className={`flex-shrink-0 px-3 py-1.5 text-xs font-medium whitespace-nowrap ${idx === currentMonthIdx ? 'border-b-2 border-indigo-600 text-indigo-700' : 'text-gray-500 hover:text-gray-700'}`}>
          ...
          <div className="p-3">
            ...
            <div className="grid grid-cols-7 gap-px bg-gray-200">
              {Array.from({ length: startOffset }).map((_, i) => <div key={`e-${i}`} className="bg-gray-50 h-20" />)}
              {Array.from({ length: totalDays }).map((_, i) => {
                const day = i + 1, dateStr = isoDate(cy, cm, day);
                const dayEvs  = eventsByDate.get(dateStr) || [];
                const isToday = dateStr === new Date().toISOString().slice(0, 10);
                const isActive = quickAdd?.date === dateStr;
                return (
                  <div key={day} onClick={e => openQuickAdd(dateStr, e)}
                    className={`bg-white h-20 p-1 transition-colors ${readOnly ? 'cursor-default' : 'cursor-pointer hover:bg-indigo-50'} ${isActive ? 'bg-indigo-50 ring-2 ring-inset ring-indigo-400' : ''}`}>
                    <div className={`text-xs font-semibold mb-0.5 w-5 h-5 flex items-center justify-center rounded-full ${isToday ? 'bg-indigo-600 text-white' : 'text-gray-700'}`}>{day}</div>
                    <div className="space-y-0.5 overflow-hidden">
                      ...
                      {dayEvs.length > 3 && <div className="text-xs text-gray-400">+{dayEvs.length - 3} more</div>}
                    </div>
                  </div>
                );
              })}
          ...
          {!readOnly && <p className="text-xs text-gray-400 text-center pb-3">Click any date to add · Click an event pill to edit</p>}
        </div>
      )}
      ...
      {!loading && view === 'list' && (
        <div className="space-y-5">
          {months.map((m, idx) => {
            ...
            const monthEvents = events.filter(ev => new Date(ev.start_date) <= new Date(y, mo + 1, 0) && new Date(ev.end_date) >= new Date(y, mo, 1));
            if (!monthEvents.length) return null;
            return (
              <div key={idx} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="px-4 py-2.5 bg-indigo-50 border-b border-indigo-100">
                  <h3 className="text-sm font-semibold text-indigo-700">{m.toLocaleString('default', { month: 'long', year: 'numeric' })}</h3>
                </div>
                <div className="divide-y divide-gray-100">
                  {monthEvents.map(ev => <EventRow key={ev.id} ev={ev} onEdit={openEdit} onDelete={remove} readOnly={readOnly} />)}
                </div>
              </div>
            );
          })}
          {events.length === 0 && (
            <div className="text-center py-16 text-gray-400">
              ...
              {!readOnly && <button onClick={openAdd} className="mt-3 text-indigo-600 text-sm hover:underline">Add the first event</button>}
            </div>
          )}
        </div>
      )}
      ...
      {/* ── QUICK-ADD POPOVER ── */}
      {quickAdd && !readOnly && (
        <div ref={quickPopoverRef} style={{ position: 'fixed', left: quickAdd.x, top: quickAdd.y, zIndex: 1000 }}
          ...
          <input autoFocus value={quickTitle} onChange={e => setQuickTitle(e.target.value)}
            onKeyDown={e => { if (e.key === 'Enter') doQuickSave(); if (e.key === 'Escape') setQuickAdd(null); }}
          ...
          <div className="flex items-center gap-1.5 mb-2.5">
            {EVENT_TYPES.map(t => (
              <button key={t.value} onClick={() => setQuickType(t.value)} title={t.label}
                className={`w-6 h-6 rounded-full flex-shrink-0 transition-all ${t.dot} ${quickType === t.value ? `ring-2 ring-offset-1 ${t.ring} scale-110` : 'opacity-40 hover:opacity-80'}`} />
            ))}
          ...
          <div className="flex items-center gap-2 mb-3 text-xs text-gray-600">
            ...
            <input type="date" value={quickEndDate} min={quickAdd.date} onChange={e => setQuickEndDate(e.target.value)}
              className="border border-gray-300 rounded px-2 py-0.5 text-xs flex-1" />
          </div>
          <div className="flex items-center justify-between">
            <button onClick={openMoreOptions} className="text-xs text-indigo-500 hover:text-indigo-700 font-medium">More options →</button>
            <div className="flex gap-2">
              <button onClick={() => setQuickAdd(null)} className="text-xs text-gray-400 hover:text-gray-600 px-2 py-1 rounded">Cancel</button>
              <button onClick={doQuickSave} disabled={!quickTitle.trim() || quickSaving}
        ...
        <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4">
          <div ref={templatePopoverRef} className="bg-white rounded-xl shadow-2xl w-72">
            <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-200">
              ...
              <span className={`ml-auto text-xs px-2 py-0.5 rounded-full border ${TYPE_MAP[templateAdd.event_type]?.color}`}>{TYPE_MAP[templateAdd.event_type]?.label}</span>
            </div>
            <div className="p-4 space-y-3">
              <div>
                <label className="text-xs font-medium text-gray-600 block mb-1">{templateAdd.multi_day ? 'Start Date' : 'Date'} *</label>
                <input type="date" autoFocus value={tmplStart}
                  onChange={e => { setTmplStart(e.target.value); if (!templateAdd.multi_day) setTmplEnd(e.target.value); }}
                  onKeyDown={e => { if (e.key === 'Enter' && !templateAdd.multi_day) doTmplSave(); }}
              ...
              {templateAdd.multi_day && (
                <div>
                  <label className="text-xs font-medium text-gray-600 block mb-1">End Date *</label>
                  <input type="date" value={tmplEnd} min={tmplStart} onChange={e => setTmplEnd(e.target.value)}
                    onKeyDown={e => { if (e.key === 'Enter') doTmplSave(); }}
                    className="w-full border border-gray-300 rounded px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" />
                </div>
              )}
            </div>
            <div className="flex justify-end gap-2 px-4 pb-4">
              <button onClick={() => setTemplateAdd(null)} className="text-sm text-gray-500 hover:text-gray-700 px-3 py-1.5">Cancel</button>
              <button onClick={doTmplSave} disabled={!tmplStart || tmplSaving}
                className="text-sm bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-1.5 rounded-lg font-medium disabled:opacity-50">
                {tmplSaving ? 'Adding…' : 'Add'}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[85vh] flex flex-col">
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 flex-shrink-0">
              {manageMode === 'form' ? (
                <button onClick={() => setManageMode('list')} className="flex items-center gap-1.5 text-sm text-gray-600 hover:text-gray-900 font-medium">
                  ← Back
                </button>
              ) : (
                <h2 className="text-base font-semibold text-gray-800">Manage Quick-Add Templates</h2>
              )}
              <button onClick={() => setManageOpen(false)} className="text-gray-400 hover:text-gray-700 text-xl leading-none">✕</button>
            </div>

            {/* ── List view ── */}
            {manageMode === 'list' && (
              <div className="flex-1 overflow-y-auto p-4">
                {tmplCategories.length === 0 ? (
                  <div className="text-center py-10 text-gray-400">
                    <p className="text-sm mb-3">No templates yet.</p>
                    <button onClick={() => openTmplForm()} className="text-indigo-600 text-sm hover:underline">Add your first template</button>
                  </div>
                ) : (
                  <div className="space-y-5">
                      ...
                      <div key={cat}>
                        ...
                        <div className="space-y-1">
                          {templates.filter(t => t.category === cat).map(t => {
                            const td = TYPE_MAP[t.event_type];
                            return (
                              <div key={t.id} className="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-gray-50 group">
                                <span className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${td?.dot ?? 'bg-gray-400'}`} />
            ...
            {manageMode === 'form' && (
              <div className="flex-1 overflow-y-auto p-5 space-y-4">
                <h3 className="text-sm font-semibold text-gray-700">{editingTmpl ? 'Edit Template' : 'Add New Template'}</h3>

                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Title *</label>
                  <input autoFocus value={tmplForm.title} onChange={e => setTmplForm(f => ({ ...f, title: e.target.value }))}
                    placeholder="e.g. Dasara Vacation, Holi, Exam-Revision Day"
                ...
                <div>
                  ...
                  <input value={tmplForm.category} onChange={e => setTmplForm(f => ({ ...f, category: e.target.value }))}
                    list="cat-list" placeholder="Holiday / Exam / School — or type a new one"
                    className="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" />
                  <datalist id="cat-list">
                    {existingCategories.map(c => <option key={c} value={c} />)}
                  </datalist>
                  <p className="text-xs text-gray-400 mt-1">Pick an existing category or type a new name to create one.</p>
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {EVENT_TYPES.map(t => (
                      <button key={t.value} onClick={() => setTmplForm(f => ({ ...f, event_type: t.value }))}
                        className={`flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full border font-medium transition-all ${
                          tmplForm.event_type === t.value ? t.color + ' ring-2 ring-offset-1 ' + t.ring : 'bg-gray-50 text-gray-600 border-gray-200 hover:bg-gray-100'}`}>
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Default Date <span className="font-normal text-gray-400">(MM-DD, optional)</span></label>
                  <input value={tmplForm.default_mmdd} onChange={e => setTmplForm(f => ({ ...f, default_mmdd: e.target.value }))}
                    placeholder="e.g. 08-15 for Independence Day"
                    className="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" />
                  <p className="text-xs text-gray-400 mt-1">If set, the date will be pre-filled when using this chip.</p>
                </div>

                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={tmplForm.multi_day} onChange={e => setTmplForm(f => ({ ...f, multi_day: e.target.checked }))}
                    className="w-4 h-4 text-indigo-600 border-gray-300 rounded" />
                  <span className="text-sm text-gray-700">Multi-day event <span className="text-gray-400 text-xs">(shows start + end date picker)</span></span>
                </label>
              </div>
            )}
            ...
            <div className="px-5 py-3 border-t border-gray-200 flex-shrink-0">
              ...
              {manageMode === 'list' ? (
                ...
                <>
                  <button onClick={() => setManageMode('list')} className="px-4 py-2 text-sm text-gray-600 hover:text-gray-800">Cancel</button>
                  <button onClick={saveTmplForm} disabled={!tmplForm.title.trim() || !tmplForm.category.trim() || tmplFormSaving}
                    className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm rounded-lg font-medium disabled:opacity-50">
                    {tmplFormSaving ? 'Saving…' : editingTmpl ? 'Save Changes' : 'Add Template'}
                  </button>
                </>
              )}
              </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
              <h2 className="text-base font-semibold text-gray-800">{modal.mode === 'add' ? 'Add Event' : 'Edit Event'}</h2>
            ...
            <div className="p-5 space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {EVENT_TYPES.map(t => (
                    <button key={t.value} onClick={() => setForm(f => ({ ...f, event_type: t.value }))}
                      className={`flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full border font-medium transition-all ${
                        form.event_type === t.value ? t.color + ' ring-2 ring-offset-1 ' + t.ring : 'bg-gray-50 text-gray-600 border-gray-200 hover:bg-gray-100'}`}>
                      <span className={`w-2 h-2 rounded-full ${t.dot}`} />{t.label}
                    </button>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={form.applies_to} onChange={e => setForm(f => ({ ...f, applies_to: e.target.value, grade: '' }))}
                    className="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400">
...
function EventRow({ ev, onEdit, onDelete, readOnly = false }: {
  ev: CalendarEvent; onEdit: (ev: CalendarEvent) => void;
  onDelete: (id: string) => void; readOnly?: boolean;
}) {
  const t = TYPE_MAP[ev.event_type];
  const start = ev.start_date.slice(0, 10), end = ev.end_date.slice(0, 10);
    ...
    <div className="flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 group">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-gray-500">
          {dateLabel}
          {ev.applies_to === 'grade' && ev.grade != null && <span className="ml-2 text-indigo-500">Grade {ev.grade}</span>}
        </p>
      </div>
